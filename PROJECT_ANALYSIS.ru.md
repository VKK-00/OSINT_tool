...
- pwnedOrNot adapter: `pwnedornot -e <email> -n` и parser upstream stdout для HIBP breach summary/breach rows без password dump lookup по умолчанию;
...
- Blackbird adapter: `<BLACKBIRD_PYTHON|python> blackbird.py --username|--email <value> --json --no-update --timeout 30` из `BLACKBIRD_DIR`, fresh JSON export ingestion и parser stdout found-lines для username/email account discovery;
- DetectDee adapter: `DetectDee detect -n|-e|-p <value> -f <DETECTDEE_DATA> -o <temp>` для username/email/phone account discovery без upstream screenshot/token/credential-stuffing flows;
...
- theHarvester adapter: `theHarvester -d <domain> -b all -f <temp.json>` и parser generated JSON/stdout для emails, hosts, URLs, API endpoints, IPs, ASNs и people fields; при наличии upstream/stash/API source attribution она сохраняется в `source_label`;
- BBOT adapter: `bbot -t <target> -p subdomain-enum -rf passive -o <tempdir> -n osint-toolkit`, Windows/Docker fallback `docker run --rm -v <output_dir>:/root/.bbot/scans -v <config_dir>:/root/.config/bbot blacklanternsecurity/bbot:stable ...`, отдельные variants `bbot-passive-web` (`subdomain-enum web-basic -rf passive -ef active aggressive deadly portscan web-screenshots`) и `bbot-passive-email` (`email-enum -rf passive -ef active aggressive deadly portscan web-screenshots`), parser generated JSON/NDJSON/stdout events для DNS names, emails, URLs, IPs, ports, technologies и findings;
...
- adapter runtime route layer: внешние adapters получают единый selected route; BBOT на Windows может перейти с native executable на Docker route, если native runtime probe падает;
...
- `osint_toolkit/image_runner.py`
  ...
  - Tesseract OCR stdout и zbarimg raw payloads разбираются в structured parser findings с line/payload counts, decoded clues и derived targets.
...
- `osint_toolkit/adapter_parsers.py`
  ...
  - Поддерживает `soxoj/maigret` NDJSON/simple JSON/CSV reports: site/status/url/tags/ids, site metadata, probe URL, rank/similarity flags и public dossier fields нормализуются в `Finding`.
  ...
  - Поддерживает `thewhiteh4t/pwnedOrNot` stdout: breach status, total breaches и HIBP breach rows нормализуются в `Finding`; dump/password values редактируются.
  ...
  - Поддерживает `Yvesssn/DetectDee` generated result/stdout lines: identity/site/profile URL нормализуются в `candidate` findings для username/email/phone targets.
  ...
  - Поддерживает `laramies/theHarvester` generated JSON/stdout output: emails, hosts/vhosts, URLs, API endpoints, IPs, ASNs и people fields нормализуются в `Finding`; `resource/type/source` rows, `*_by_source` maps и nested `source_results` сохраняют provider/source в `source_label`.
  - Поддерживает `blacklanternsecurity/bbot`, `blacklanternsecurity/bbot-passive-web` и `blacklanternsecurity/bbot-passive-email` generated JSON/NDJSON/stdout events: `DNS_NAME`, `EMAIL_ADDRESS`, `URL`, `IP_ADDRESS`, `OPEN_TCP_PORT`, `TECHNOLOGY`, `FINDING` и `VULNERABILITY` нормализуются в `Finding`.
  - Поддерживает `smicallef/spiderfoot` JSON/stdout events: `INTERNET_NAME`, `DOMAIN_NAME`, `EMAILADDR`, `WEBLINK`, `IP_ADDRESS`, `TCP_PORT_OPEN`, `PHONE_NUMBER`, `HUMAN_NAME`, `TECHNOLOGY`, ASN и vulnerability/finding events нормализуются в `Finding`; parser сохраняет `target_kind`/`target_value` для domain/url/email/phone/username runs.
- `osint_toolkit/adapter_setup.py`
  - `AdapterSetup` — readiness/install/config view для adapter.
  - `build_adapter_setup()` — проверка executable в `PATH`, install command, docs URL, env readiness, env-backed venv executables, selected execution route и allowlisted executable identity probes.
- `osint_toolkit/adapter_runtime.py`
  - `resolve_adapter_runtime()` — выбирает native или fallback route для adapter; сейчас закрывает BBOT Docker route на Windows при native `fcntl`/POSIX startup failure.
  - `render_adapter_command()` / `render_adapter_output_dir_args()` — строят команду и output args с учётом route, включая Docker volume mounts для BBOT generated reports и host config.
- `osint_toolkit/adapter_probe.py`
  - `probe_adapter_executable()` — лёгкие проверки неоднозначных executables; Subfinder, ProjectDiscovery `httpx`, Amass, theHarvester, BBOT и PhoneInfoga проверяются по help markers, чтобы unrelated binaries не считались ready.
...
- `osint_toolkit/toolbox_server.py`
  ...
  - `ToolboxRequestHandler` — HTTP endpoints `/api/search`, `/api/profiles`, `/api/profiles/save`, `/api/profiles/delete`, `/api/tools`, `/api/jobs`, `/api/jobs/<id>`, `/api/jobs/<id>/report`, `/api/cases`, `/api/cases/export`, `/api/cases/<id>`, `/api/cases/<id>/sources`, `/api/cases/<id>/export`, `/api/cases/<id>/graph`, `/api/cases/<id>/update`, `/api/cases/<id>/delete`, `/api/case-index`, `/api/case-path`, `/api/case-network`, `/api/health`.
...
- `osint_toolkit/case_export.py`
  - `export_case_package()` — пишет handoff-пакет saved case: JSON/Markdown, CSV findings/sources/targets/entities/edges, metadata, graph summary, manifest с SHA-256 и optional zip.
  - `export_cases_package()` — пишет bulk handoff-пакет по списку saved case payloads: подпапка на кейс, `bulk_manifest.json` и optional zip всего bulk directory.
- `osint_toolkit/cli.py`
  - argparse CLI: `stats`, `catalog`, `show`, `scan`, `search`, `profiles`, `adapters`, `adapter-profiles`, `adapter-setup`, `doctor`, `tools doctor/install-plan/install/env`, `run-adapter`, `toolbox`, `investigate`, `cases`, `case-show`, `case-sources`, `case-export`, `cases-export`, `case-update`, `case-delete`, `case-graph`, `case-index`, `case-path`, `case-network`, `recommend`, `brief`.
...
7. Case Browser читает `/api/cases` с optional workflow/profile/scope filters, `/api/cases/export`, `/api/cases/<id>`, `/api/cases/<id>/sources`, `/api/cases/<id>/export` и `/api/cases/<id>/graph`, рисует bounded SVG-граф из сохранённых `entities`/`edges`, показывает source summary по сохранённым findings, пишет single-case или filtered bulk handoff-пакет и показывает summary/focus analysis рядом с JSON; клик или Enter/Space на узле заполняет focus entity и перезапрашивает соседей.
...
8. Runner обогащает parsed findings общей execution metadata (`adapter_repository`, `adapter_command`, `adapter_execution_route`, `adapter_executable_path`, `adapter_returncode`, `adapter_started_at`, `adapter_finished_at`, `adapter_duration_ms`, `adapter_timeout_seconds`, `adapter_generated_output_files`, `parser_version`), чтобы case-store и JSON report могли показывать не только сигнал, но и точный запуск, который его породил.
9. Restricted adapters требуют отдельный `--allow-restricted`.
...
5. Пользователь запускает `python -m osint_toolkit case-export --case-db <path> <case_id> --out <dir> --zip`.
6. `export_case_package()` пишет handoff directory и optional zip без изменения SQLite.
7. Пользователь запускает `python -m osint_toolkit cases-export --case-db <path> --workflow ... --out <dir> --zip`.
8. `CaseStore.list_cases()` применяет фильтры, `CaseStore.load_case()` загружает matched cases, а `export_cases_package()` пишет bulk directory и optional zip.
9. Пользователь запускает `python -m osint_toolkit case-update --case-db <path> <case_id> --title ... --scope-note ...`.
10. `CaseStore.update_case()` меняет только title и metadata, не пересчитывая findings/entities/edges.
11. Пользователь запускает `python -m osint_toolkit case-delete --case-db <path> <case_id> --yes`.
12. `CaseStore.delete_case()` удаляет кейс и дочерние rows через SQLite cascade.
13. Пользователь запускает `python -m osint_toolkit case-graph --case-db <path> <case_id>`.
14. `analyze_case_graph()` строит summary сохранённого графа и, при указанном фокусе, возвращает соседей конкретной сущности.
15. Пользователь запускает `python -m osint_toolkit case-index --case-db <path>`.
16. `CaseStore.list_entity_index()` строит индекс сущностей по всем сохранённым кейсам; `find_cases_by_entity()` показывает кейсы для точной сущности.
17. Пользователь запускает `python -m osint_toolkit case-path --case-db <path> --from-kind ... --to-kind ...`.
18. `CaseStore.load_cases()` загружает bounded набор saved cases, а `analyze_cross_case_path()` строит объединённый graph и возвращает weighted shortest path с provenance по каждому hop.
19. Пользователь запускает `python -m osint_toolkit case-network --case-db <path>`.
20. `analyze_cross_case_network()` агрегирует одинаковые graph edges между saved cases, считает degree/case_count и отдаёт bounded visible subgraph для CLI/API/toolbox.
...
`ToolboxSection[] + AdapterProfile[] -> render_toolbox_html() -> local HTML -> operator fills image path/seeds/profile fields -> copy-ready command OR /api/profiles|/api/profiles/save|/api/profiles/delete|/api/tools|/api/search -> ToolboxJobRunner -> readiness view OR python -m osint_toolkit search -> report/case -> Case Browser /api/cases|cases/export|sources|export|graph|update|delete|case-index|case-path|case-network`
...
`domain/url seed -> domain-recon/bbot-passive-web/bbot-passive-email/broad-recon profile -> subfinder/httpx/passive amass/theHarvester/BBOT/SpiderFoot/Argus -> JSONL/plain stdout/generated JSON/events/stdin-driven output -> subdomain/email/phone/url/domain/ip/port/technology metadata -> Entity[]/GraphEdge[]`
...
`InvestigationResult + workflow/profile/scope policy metadata -> CaseStore(SQLite) -> cases/case-show/case-sources/case-export/cases-export/case-graph/case-index/case-path/case-network -> table/Markdown/CSV/JSON + handoff packages`
...
- `BBOT_RUNNER` — optional override `auto|native|docker`; на Windows auto использует Docker fallback, если native BBOT не проходит runtime probe.
- `BBOT_DOCKER_IMAGE` — optional Docker image/tag для BBOT route, default `blacklanternsecurity/bbot:stable`.
- `BBOT_DOCKER_CONFIG_DIR` — optional host-папка для BBOT Docker `/root/.config/bbot`; default `~/.config/bbot`.
...
- `DETECTDEE_DATA` — обязательный путь к upstream `data.json` для DetectDee detect adapter.
...
python -m osint_toolkit case-export --case-db cases.sqlite case-001 --out reports/case-001-export --zip
...
- Case management intentionally narrow: `case-update` и `/api/cases/<id>/update` меняют только title/scope_note, `case-export`/`cases-export` и `/api/cases/<id>/export`/`/api/cases/export` пишут read-only handoff packages внутри выбранной output-папки, а `case-delete` и `/api/cases/<id>/delete` требуют явного подтверждения; нет bulk delete, raw SQL editor или редактирования saved findings/entities/edges из UI.
...
- Graph edges покрывают базовые отношения, включая `email -> domain`, `email/domain -> provider`, `email/domain -> subdomain`, `domain -> email`, `domain -> phone`, `domain -> discovered/social/sitemap URL`, `domain -> robots disallow path`, `domain -> subdomain`, `domain -> registrar`, `domain -> nameserver`, `domain -> whois-server`, `domain -> ip|port|technology`, `url -> instagram`, `url -> social-profile`, `instagram -> platform/display name/account id/public URLs`, `social -> social-profile/platform/display name/account id/public URLs` и adapter-derived `email -> related_email`; есть summary/focus-neighbor analytics, cross-case entity index, weighted shortest path, bounded cross-case network, command toolbox, served Case Browser, clickable bounded SVG graph и client-side graph filters по entity kind/value, relation и тексту. Полноценный advanced graph layout/analytics UI ещё не реализован.
...
- 2026-06-25: Maigret parser расширен до richer dossier fields: site metadata, probe URL, rank/similarity flags, contact/profile fields, activity counters, external/profile image URLs и related usernames теперь попадают в metadata/entities/graph.
...
- 2026-06-25: theHarvester parser расширен source attribution: `resource/type/source` rows, `*_by_source` maps и nested `source_results` provider payloads сохраняют источник в `source_label` metadata/evidence; стандартный generated JSON без per-item source продолжает обрабатываться как агрегированный отчёт.
- 2026-06-25: добавлен `blacklanternsecurity/bbot` external adapter в passive `subdomain-enum` режиме: runner задаёт временный `-o`/`-n`, parser читает generated JSON/NDJSON/stdout events и нормализует DNS names, emails, URLs, IPs, ports, technologies, findings и vulnerabilities; graph/entity mapping расширен сущностями `ip`, `port` и `technology`.
...
- 2026-06-25: `Owez/yark` adapter route исправлен на `yark new <archive_name> <channel_url>` во временной директории; parser читает generated `yark.json` и нормализует channel/video archive clues, description contacts и YouTube video URLs.
- 2026-06-25: добавлен `iojw/socialscan` username/email adapter в `username-full` и `email-safe`: runner пишет generated JSON, parser нормализует availability/usage signals в `candidate`/`not_found`/`skipped`/`error` без выдачи результата за confirmed identity.
...
- 2026-06-25: добавлен `Yvesssn/DetectDee` external adapter: runner требует `DETECTDEE_DATA`, запускает upstream `DetectDee detect -n|-e|-p ... -f <data.json> -o <temp>`, а parser нормализует generated result/stdout profile hits для username/email/phone без включения screenshot/token/credential-stuffing flows.
...
- 2026-06-25: BBOT readiness усилен runtime dry-run probe: установленный executable больше не считается `ready`, если upstream Scanner startup падает до выполнения scan, например на native Windows из-за `ansible_runner`/`fcntl`.
...
- 2026-06-25: SpiderFoot parser получил `target_kind`/`target_value` provenance metadata и regression fixtures для email, phone и username target modes.
