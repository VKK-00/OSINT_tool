...
- 2445 активных публичных URL/check-шаблонов профилей: 46 curated правил, 479 валидных записей из Sherlock `data.json`, 715 WhatsMyName `wmn-data.json` GET/POST entries и 1906 sanitized Maigret site rules после дедупликации одинаковых URL;
...
- username permutation/alias strategy уже покрывает common given-name aliases, bundled RU/UA alias resource, handle suffixes, operator-provided alias dictionaries и deterministic platform-aware scoring/hints; alias resource расширяемый, но не является полным историческим реестром имён;
...
- explicit passive email-enum adapter variant для `blacklanternsecurity/bbot-passive-email`: `bbot -t <target> -p email-enum -rf passive -ef active aggressive deadly portscan web-screenshots -o <tempdir> -n osint-toolkit`;
...
- adapter profile `bbot-passive-email` для explicit passive BBOT `email-enum` route без active/aggressive/deadly/portscan/screenshot flags.
...
- нет активных Amass/bruteforce modes, BBOT active/deadly/portscan/screenshot modules, broader SpiderFoot/Argus use cases, theHarvester screenshots или active API endpoint scans по умолчанию.
...
- adapter profile `bbot-passive-email` for the explicit passive BBOT `email-enum` route;
...
- saved-case handoff package through CLI `case-export`, served `/api/cases/<id>/export` and toolbox `Export`, including JSON/Markdown, CSV findings/sources/targets/entities/edges, graph summary, manifest checksums and optional zip;
- filtered bulk handoff package through CLI `cases-export`, served `/api/cases/export` and toolbox `Export list`, including one directory per matched case, `bulk_manifest.json` and optional zip;
...
- served toolbox Case Browser for saved cases, case detail, source summaries, single/bulk export packages, safe title/scope update, typed-confirm delete, clickable bounded SVG case graph, graph summary/focus and cross-case index through token-protected allowlisted endpoints;
