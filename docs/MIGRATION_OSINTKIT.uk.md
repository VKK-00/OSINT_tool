...
Статус: **етапи 0–1 завершені; етапи 2–4 виконані частково** (див. чекліст унизу).
Мета — прибрати дублювання між двома пакетами й залишити **один рушій** та один UI-шар.
...
- [x] Жоден модуль не існує в двох реалізаціях; дублі `dorks/telegram/username` — делегування через `osintkit/bridge.py`; транслітерація — єдина в `osint_toolkit/translit.py`; username-sites DB — єдина (`osint_toolkit.sites`, curated UA/RF-сайти перенесені туди).
